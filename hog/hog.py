...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while y > 0:
        z = dice()
        if z != 1 and checkkk != 1:
            x = x + z
            y = y - 1
        elif z != 1 and checkkk == 1:
            y = y - 1
        elif z == 1:
            checkkk = 1
            x = 1
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    if num_rolls == 0:
        w = picky_piggy(opponent_score)
    else:
        w = roll_dice(num_rolls, dice) 
    return w
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while (score0 < goal and score1 < goal):
        ...
        if who == 0:
            w = strategy0(score0, score1)
            score0 = score0 + take_turn(w, score1, dice, goal)
            score0 = score0 + hog_pile(score0, score1)
            
        else:
            z = strategy1(score1, score0)
            score1 = score1 + take_turn(z, score0, dice, goal)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def commentary(score0, score1):
        prev = last_score
        current = running_high
        number_change = 0
        if who == 0:
            number_change = score0 - prev
            if number_change > current:
                current = number_change
                prev = score0
                print(f"{number_change} point(s)! That's a record gain for Player {who}!")
            else:
                prev = score0
                
        else:
            number_change = score1 - prev
            if number_change > current:
                current = number_change
                prev = score1
                print(f"{number_change} point(s)! That's a record gain for Player {who}!")
        ...
        return announce_highest(who, prev, current)
...
def make_averaged(original_function, trials_count=1000):
    ...
    def average(*args):
        ...
        while i < trials_count:
            temp = temp + original_function(*args)
            quotient = temp / trials_count
            i += 1
        return (quotient)
    return(average)
    # END PROBLEM 8
...
def picky_piggy_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    return_val = 0
    if (picky_piggy_score >= cutoff):
        return_val = 0
    else:
        return_val = num_rolls
    return(return_val)
    # END PROBLEM 10
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    hog_pile_score = picky_piggy(opponent_score)+score
    return_val = 0
    if (hog_pile_score == opponent_score):
        return_val = 0
    else:   
        return_val = picky_piggy_strategy(score, opponent_score, cutoff, num_rolls)
    return(return_val)
