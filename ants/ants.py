...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def help(place,minr,maxr,distance):
            if place.is_hive:
                return None
            elif place.bees and (minr <= distance <= maxr):
                return random_bee(place.bees)
            else:
                place = place.entrance
                distance += 1
                return help(place,minr,maxr,distance)
        
        return help(self.place, self.min_range, self.max_range,0)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        #damage all bees in list, reduce their health by amount
        bee = 0
        if self.health - amount <= 0:
            temp = amount + self.damage
            for bee in list(self.place.bees):
                bee.reduce_health(temp)
...
class HungryAnt(Ant):
    ...
    def __init__(self,chew_countdown = 0,health=1):
        super().__init__(health)
        self.chew_countdown = chew_countdown

    def consume(self,countdown):
        if self.place.bees:
            if countdown <= 0:
    ...
    def action(self, gamestate):
        self.consume(self.chew_countdown)
...
class TankAnt(ContainerAnt):
    ...
    def protecc(self, amount):
        i = 0
        listy = list(self.place.bees)
        if self.place.bees:
            for i in listy:
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        if gamestate.queen == False: # Queen ant already present
    ...
    def action(self, gamestate):
        ...
        def helprally(place):
            if place.exit == None:
                ...
                place = place.exit
                helprally(place)
        return helprally(self.place.exit)
