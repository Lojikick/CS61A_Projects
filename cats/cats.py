...
def choose(paragraphs, select, k):
    ...
    def master(ps, s, k):
        for i in range(len(ps)):
            if s(ps[i]):
                true_dat.append(ps[i])
        if k >= len(true_dat):
            return(str(''))
        else:
            return true_dat[k]   
            
    return master(paragraphs, select, k)
    # END PROBLEM 1
...
def wpm(typed, elapsed):
    ...
    if typed == '':
        ...
        temp = len(typed)
        tracker_words = temp / 5
        return (tracker_words / elapsed) * 60
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
           temp = diff_function(typed_word,word, limit)
           if  least_diff > temp:
               least_diff, least_word = temp, word
    if least_diff > limit:
        return typed_word
    return least_word
    # END PROBLEM 5
...
def minimum_mewtations(start, goal, limit):
    ...
    if len(start) == 0 or len(goal) == 0:  # Fill in the condition
        ...
        # BEGIN
        return min((1 + minimum_mewtations(start, goal[1:], limit-1)), (1 + minimum_mewtations(start[1:], goal, limit-1)), (1 + minimum_mewtations(start[1:], goal[1:], limit-1)))
        # END
    ...
    def progress (sofar,prompt):
        ...
        for i in range(len(sofar)):
            if sofar[i] != prompt[i]:
                ...
                counter = counter + 1
        return counter
    counter = progress (sofar,prompt)/len(prompt)
    ...
    def looper(time):
        ...
        for i in range(len(time)):
            if i+1 == len(time):
                ...
                lister.append(time[i+1]-time[i])
                
    cool_timez = []

    for i in range(len(times_per_player)):
        cool_timez.append(looper(times_per_player[i]))
    
    return match(words,cool_timez)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    listy = [[]for player in player_indices]
    for i in word_indices:
        temp = 100
        for j in player_indices:
            if time(match,j,i)<temp:
                temp = time(match,j,i)
                word = word_at(match,i)
                index = j
        listy[index].append(word)
    return listy
